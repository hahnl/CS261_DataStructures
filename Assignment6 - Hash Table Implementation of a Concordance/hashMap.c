...
void _setTableSize(struct hashMap * ht, int newTableSize)
{
	...
	for (int i = 0; i < oldSize; i++) {
		...
		while (temp1 != 0) {
			insertMap(ht, temp1->key, temp1->value);
			temp2 = temp1;
			temp1 = temp1->next;
			free(temp2);
		}
...
void removeKey (struct hashMap * ht, KeyType k)
{
	...
	hashLink *temp1 = ht->table[index];

  if (temp1 == NULL) {
		return;
	}

	if (strcmp(temp1->key, k) == 0) {
		hashLink *temp2 = temp1->next;
		free(temp1->key);
		free(temp1);
		ht->table[index] = temp2;
		(ht->count)--;
		return;
	}

  while (temp1->next != 0) {
		if (strcmp(temp1->next->key, k) == 0) {
			hashLink *temp2 = temp1->next->next;
			free(temp1->next->key);
			free (temp1->next);
			temp1->next = temp2;
			(ht->count)--;
