...
void addHeap(DynArr *heap, TYPE val)
{
    ...
    while (last != 0) {
	     lastParent = (last-1)/2;
	     if (compare(getDynArr(heap, last), getDynArr(heap, lastParent)) == -1)
	     {
		      swapDynArr(heap, lastParent, last);
		      last = lastParent;
	     }
	     else {
				  return;
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
  /* FIXME */
	...
	if (rightChild < max) {
		int smallChild = _smallerIndexHeap(heap, leftChild, rightChild);
		if (compare(getDynArr(heap, smallChild), getDynArr(heap, pos)) == -1)	{
